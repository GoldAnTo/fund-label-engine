...
def render_report(
    *,
    ...
    if rows:
        for row in rows:
            lines.append(
                ...
                f"{row['score']:.2f} | {_join(row.get('portfolio_roles'))} | "
    ...
    return {"run_id": selected_run_id, "row_count": len(rows), "excluded_count": len(excluded)}
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "wrote {out_md} (run_id={run_id}, rows={row_count}, excluded={excluded_count})".format(
            out_md=args.out_md,
            **summary,
        )
    )
