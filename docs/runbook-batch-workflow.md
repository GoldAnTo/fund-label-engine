...
`backend/app/batch.py` 暴露的 CLI 参数：

| 参数 | 默认 | 说明 |
|---|---|---|
| `--min-nav-samples` | RuleConfig 默认 | NAV 样本数 gate；做 1Y 标签时设 180 |
| `--deep-value-weight-min` | 0.6 | 深度价值的持仓权重阈值 |
| `--quality-growth-weight-min` | 0.5 | 质量成长的持仓权重阈值 |
| `--factor-db` | 无 | 外挂股票因子 SQLite 路径 |

Makefile 当前 `run-batch` 写死的组合：
`--min-nav-samples 180 --deep-value-weight-min 0.4 --quality-growth-weight-min 0.4`。
