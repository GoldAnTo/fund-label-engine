...
| 基准日收益 | 逐只解析业绩比较基准，拆组件合成 | 多数据源拼接 | **28 只需采购** |
...
|---|---|---|
| ready | 114 | 能 |
| missing_source | 27 | 不能（需采购债券指数数据） |
...
| 相对基准标签可展示 | 114 | Alpha、Beta、超额收益、信息比率 |
| 相对基准标签暂不可展示 | 28 | 缺基准数据源 |
...
| 标签 | 触发条件 | 阈值 | 当前触发数 |
|---|---|---|---|
| excess_return_strong | 年化超额 ≥ 5% | 0.05 | 66 |
| information_ratio_high | 信息比率 ≥ 0.5 | 0.5 | 56 |
| tracking_error_high | 跟踪误差 ≥ 8% | 0.08 | 114 |
| alpha_positive | Alpha ≥ 3% | 0.03 | 88 |
| beta_high | Beta ≥ 1.2 | 1.2 | 1 |
| beta_low | Beta ≤ 0.8 | 0.8 | 105 |
| benchmark_data_missing | 基准数据缺失 | — | 28 |
...
| 卡点 | 状态 | 影响 | 解决方案 |
|---|---|---|---|
| 28 只缺基准源 | 需采购 | 不能算 Alpha/Beta | 买中债登授权 |
...
基础功能可以用了：142 只基金的标签、证据、同类排名、竞品横评都能看。3 个限制：① 28 只缺基准源；② 持仓只有一期；③ 组合草案缺相关性矩阵。

### Q2：数据从哪来？要花钱吗？

大部分数据免费已有。唯一采购需求：28 只基金的债券指数日收益数据（向中债登购买授权）。
...
**展现方式**：
- 基准数据可用（114 只）：按窗口计算 6 个指标，逐个判断触发
- 基准数据缺失（28 只）：输出 benchmark_data_missing 标签，不展示 Alpha/Beta/超额收益
